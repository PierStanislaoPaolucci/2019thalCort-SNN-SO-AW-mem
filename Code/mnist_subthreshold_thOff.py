#####################################################################################
#
# mnist_subthreshold_thOff.py
# INFN - November 2018
#
# Training, Sleep and Classification on a Thalamo-cortical model used to generate 
# the data reported in Fig.2, 3 and 4 of the following article:
#
...
nest.Simulate(t_pause*4)


print '######################################################################'
print '# Test pre-sleep'
print '######################################################################'
print
...
Accuracy_post_sleep = float(count_right)/float(i_test+1.)*100.
Accuracy_post_sleep_unsupervised = float(count_unsupervised_right)/float(i_test+1.)*100.

print 'Accuracy pre-sleep = ', Accuracy_pre_sleep, '%'
print 'Accuracy post-sleep = ', Accuracy_post_sleep, '%'

print 'Accuracy pre-sleep unsup = ', Accuracy_pre_sleep_unsupervised, '%'
print 'Accuracy post-sleep unsup = ', Accuracy_post_sleep_unsupervised, '%'
...
timetot = (t_train+t_pause)*n_train+(2*t_check+2*t_pause)*n_test+t_sleep+t_pause*(10.+4.)

DT = 10.
NT = int(round(timetot/DT))

fr_exc=np.zeros((n_exc,NT),dtype=float)
...
for i in range(n_out):
    times = []
    times_bin = []
    times = np.array(evt_sd_out[i]["times"])
    times_bin = np.floor(times / DT)
    times_bin  = times_bin.astype(int)

    fr =  np.bincount(times_bin) / DT*1000
    fr_out[i,0:len(fr)] = fr

    plt.plot(np.arange(NT)*DT,fr_out[i,:])

plt.xlabel('time (ms)')
plt.ylabel('f.r. out (Hz)')
...
for i in range(n_exc):
    ...
    plt.plot(np.arange(NT)*DT,fr_exc[i,:])

plt.xlabel('time (ms)')
plt.ylabel('f.r. exc (Hz)')
...
for i in range(n_inh):
    ...
    plt.plot(np.arange(NT)*DT,fr_inh[i,:])

plt.xlabel('time (ms)')
plt.ylabel('f.r. inh (Hz)')
plotEndTime2 = time.time()
...
for i in range(n_inp):
    ...
    plt.plot(np.arange(NT)*DT,fr_inp[i,:])

plt.xlabel('time (ms)')
plt.ylabel('f.r. inp (Hz)')

plt.subplot(515)
for i in range(n_inh2):
    times = []
    times_bin = []
    times = np.array(evt_sd_inh2[i]["times"])
    times_bin = np.floor(times / DT)
    times_bin  = times_bin.astype(int)

    fr =  np.bincount(times_bin) / DT*1000
    fr_inh2[i,0:len(fr)] = fr

    plt.plot(np.arange(NT)*DT,fr_inh2[i,:])

plt.xlabel('time (ms)')
plt.ylabel('f.r. inh2 (Hz)')
...
sys.stdout.flush()
sio.savemat('fr_exc_thfb_30train.mat', {'fr_exc':fr_exc})
sio.savemat('fr_inp_thfb_30train.mat', {'fr_inp':fr_inp})
sio.savemat('test_out.mat', {'test_out':test_out})

plt.figure()

ax1 = plt.subplot(3,1,1)
vector = np.array(w_inp_exc)
plt.hist(vector[:,2],50, facecolor='gray', alpha=0.75)
ax1.set_yscale('log')
plt.ylabel('N')

ax2 = plt.subplot(3,1,2)

vector = np.array(w_pre_sleep)
plt.hist(vector[:,2],50, facecolor='gray', alpha=0.75)
ax2.set_yscale('log')
plt.ylabel('N')
...
vector = np.array(w_post_sleep)
plt.hist(vector[:,2],50, facecolor='gray', alpha=0.75)
ax3.set_yscale('log')
plt.ylabel('N')

sio.savemat('w_pre_sleep_thfb_30train.mat', {'w_pre_sleep':w_pre_sleep})
sio.savemat('w_post_sleep_thfb_30train.mat', {'w_post_sleep':w_post_sleep})
sio.savemat('accuracy_thfb_30train.mat', {'Accuracy_pre_sleep':Accuracy_pre_sleep_unsupervised,'Accuracy_post_sleep':Accuracy_post_sleep_unsupervised})
